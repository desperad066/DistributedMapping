...
partitionGraph(gtsam::NonlinearFactorGraph graph,
  ...
  if(useMetisPartitioning){
    ...
    std::cout << "don't use METIS" << std::endl;
    ...
    for(size_t sub_id =0; sub_id < num_subgraphs; sub_id++){ // for each subgraph
      ...
      factor_id = init_pose;
      std::cout << "ini:" << init_pose << "final:" << final_pose << std::endl;
      for (int i = init_pose ; i < final_pose; ) { // for each factor in the original graph
        ...
        if(std::fabs(key0Index - key1Index)==1){  // Increase i only when its an odometry constraint
          i++;
        }
...
/********************************************************************************************/
// Calculate the seperators
void createSubgraphInnerAndSepEdges(const char robotName_, const int index, const gtsam::NonlinearFactorGraph& subgraph,
                                    std::vector<std::pair<int, gtsam::Symbol> > &seperators,
                                    std::vector<gtsam::Pose3> &measure_,
                                    bool useLandmarks_ = false, bool isDebug = false){

  size_t factorSize = subgraph.size();
  for(size_t k=0; k < factorSize; k++){ // this loops over the factors in subgraphs[i]
    ...
    gtsam::KeyVector keys = subgraph.at(k)->keys();
    ...
    if (robot0 == robot1 || (useLandmarks_ && robot1 == toupper(robot0))){ // keys from the same subgraph
      if(isDebug)
          std::cout << "Factor connecting (intra): " << robot0 << " " << symbolIndex(key0) << " " <<  robot1 << " " << symbolIndex(key1) << std::endl;
    }
    else{
      // Add it as a prior factor using the current estimate from the other graph
      if(isDebug)
          std::cout << "Factor connecting (extra): " << robot0 << " " << symbolIndex(key0) << " " <<  robot1 << " " << symbolIndex(key1) << std::endl;

      // Neighbors data structure
      if(robot0 == robotName_ || (useLandmarks_ && robot0 == toupper(robotName_))){
          std::pair<int, gtsam::Symbol> from(index, key0);
          std::pair<int, gtsam::Symbol> to(index + (robot1 - robot0), key1);
          seperators.push_back(from);
          seperators.push_back(to);

          boost::shared_ptr<gtsam::BetweenFactor<gtsam::Pose3>> factor =
              boost::dynamic_pointer_cast<gtsam::BetweenFactor<gtsam::Pose3> >(subgraph.at(k));
          measure_.push_back(factor->measured());
      }
      else{
//          防止多次計算
//          seperators.push_back(index-(robot1-robot0));
//          seperators.push_back(key0);
//          seperators.push_back(index);
//          seperators.push_back(key1);
      }
    }
  }
}
...
//  std::cout << "Centralized error LM " << graph.error(LMresult) << std::endl;
  ...
  for(size_t sub_id =0; sub_id < sub_results.size(); sub_id++){
    std::string filepath = output_dir + "/" + boost::lexical_cast<std::string>(sub_id) + "_optimized.g2o";
