...
class ADMM
{
  public:
    ...
    /**
     * @brief createSubgraphInnerAndSepEdges
     * @param
     */
    std::pair<gtsam::NonlinearFactorGraph, std::vector<size_t> > createSubgraphInnerAndSepEdges(const gtsam::NonlinearFactorGraph& subgraph);
    ...
    void load(std::vector< gtsam::NonlinearFactorGraph > subgraphs,
      ...
      uall_k = std::vector<double>(6 * separators_.size() / 2, 0.0f); // scaled dual variables: u_k = y_k/rho (scaling by 1/rho won't have any effect initially since y_k = 0)
      measures_ = std::vector<gtsam::Pose3>(separators_.size() / 2, gtsam::Pose3());
      for(size_t index = 0; index < separators.size(); index+=2){
          size_t fromIndex = separators[index].first;
          gtsam::Key fromKey = separators[index].second;
          gtsam::Pose3 fromPose = subinitials[fromIndex].at<gtsam::Pose3>(fromKey);
          size_t toIndex = separators[index+1].first;
          gtsam::Key toKey = separators[index+1].second;
          gtsam::Pose3 toPose = subinitials[toIndex].at<gtsam::Pose3>(toKey);
          gtsam::Pose3 between = fromPose.between(toPose);
          measures_[index/2] = between;
      }
    }
  ...
  protected:
    ...
    std::vector<std::pair<int, gtsam::Symbol> >  separators_; // separators in the form <sepkey0, subgraph_id01, subgraph_id02, sepkey1, subgraph_id11, subgraph_id12, ...>
